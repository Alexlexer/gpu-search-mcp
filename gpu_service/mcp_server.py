...
def diagnostics_snapshot() -> dict:
    ...
    decision_config = load_decision_model_config()
    return {
        "version": VERSION,
        "status": status,
        "device": device,
        ...
        "capabilities": {
            ...
            "mcpTools": True,
            "decisionLayer": decision_config["provider"] != "disabled",
        },
        "decisionModel": {
            "provider": decision_config["provider"],
            "configured": decision_config["provider"] != "disabled",
        },
...
def _make_planner():
    ...
    return ChangePlanner(index, semantic, deps, symbols, git_state, _make_decision_model())


def _make_decision_model():
    from decision_model import DeterministicDecisionModel, LocalDecisionModel, TypeSafeJevDecisionModel
    config = load_decision_model_config()
    if config["provider"] == "local" and config["base_url"] and config["model"]:
        model = LocalDecisionModel(config["base_url"], config["model"], config["api_key"])
        model.confidence_threshold = config["confidence_threshold"]
        return model
    if config["provider"] == "typesafe" and config["model"] and config["api_key"]:
        model = TypeSafeJevDecisionModel(config["model"], config["api_key"], config["base_url"] or "https://api.typesafe.ai")
        model.confidence_threshold = config["confidence_threshold"]
        return model
...
def _parse_args(argv=None):
    ...
    parser.add_argument(
        "--decision-model", choices=["disabled", "deterministic", "local", "typesafe"],
        default=None, help="Optional bounded context-decision provider; credentials stay in environment.",
    )
...
def _prepare_startup(args):
    ...
    if getattr(args, "decision_model", None):
        os.environ["GPU_SEARCH_DECISION_MODEL"] = args.decision_model
